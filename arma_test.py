...
def predict_using_online_mode(
    ...
    # return avg metrics
    avg_metrics = dict()
    for metric_name in model_error_metrics.keys():
        if len(model_error_metrics[metric_name]) > 0:
            avg_metrics[metric_name] = sum(model_error_metrics[metric_name]) / len(model_error_metrics[metric_name])
...
def sigmoid_test(logger, is_online, should_plot):
    logger.log('load records..')
    data_records = utils.parse_csv(DATASET_FILE_PATH, smoothing_level=1, should_shuffle=False)
                ...
                for i in range(NUMBER_OF_PREDICTIONS_AHEAD):
                    next_prediction = \
                        SigmoidCurve.fit_and_predict(
                            train_set + test_set[:i], 1, mid_max_rate=mid_max_rate
                        )[0]
                    sigmoid_predictions.append(next_prediction)
                    mid_max_rate *= 0.9
                    mid_max_rate = max(1.2, mid_max_rate)
    ...
    for metric_name in model_error_metrics.keys():
        if len(model_error_metrics[metric_name]) > 0:
            avg_metric = sum(model_error_metrics[metric_name]) / len(model_error_metrics[metric_name])
...
if __name__ == '__main__':
    ...
    sigmoid_test(main_logger, is_online=True, should_plot=False)
